...
let player1 = {
    ...
    increaseImgTimer: function (timeDifferential) {
        this.imgTimer += timeDifferential;
    ...
    isIdle: function () {
        return this.element.html() == ('<img class="pl" src=' + blobImages.idleImg + ' />');
    },

    changeFrame: function (newImgSrc) {

        //Reset the image timer if the new frame is not an idle frame.
        if (newImgSrc != blobImages.idleImg) {
            this.element.html('<img class="pl" src=' + blobImages.idleImg + ' />');
            this.resetImgTimer();
        }

        this.element.html('<img class="pl" src=' + newImgSrc + ' />');
        // console.log("Changing image");
    },

    specialIcon: $("#PHUD-1 .SpecialIcon"),

    changeSpecialIcon: function(newImgSrc){
        this.specialIcon.html('<img class="iconImg" src=' + newImgSrc + ' />');
    }
};
...
let attack = {
    ...
    slashShort: function (playerA, playerB) {
        ...
        playerA.changeFrame(blobImages.shortRangeImg);
    ...
    slashLong: function (playerA, playerB) {
        ...
        playerA.changeFrame(blobImages.longRangeImg);
    ...
    slashWide: function (playerA, playerB) {
        ...
        playerA.changeFrame(blobImages.wideRangeImg);
...
let gameStates = {
    ...
    states: ["preGame", "onGoing", "paused", "gameOver"],
    currentState: "",

    changeStateToOnGoing: function () {
    ...
    changeStateToGameOver: function () {
        this.currentState = this.states[3];
    },

    endGame: function (winner, loser) {

        
        winner.changeFrame(blobImages.winImg);
        loser.changeFrame(blobImages.loseImg);

        
        this.changeStateToGameOver();

        alert("Game over!");
        alert("Thank you for playing!\nTo play again, refresh the page (press F5).")
    }
}

gameStates.changeStateToOnGoing();
// console.log("current state: " + gameStates.currentState);
...
function gameLoop() {
    ...
    if (gameStates.currentState == "onGoing") {

        //Handle pausing
        {
            if (gamepads.length < 2) {
                // console.log("A gamepad was diconnected, pausing...");
                gameStates.changeToPaused();
            } else if (pauseButtonClicked(gamepads)) {
        ...
        if (player1.isAlive()) {
            ...
            gameStates.endGame(player2, player1);
        }
        ...
        if (player2.isAlive()) {
            ...
            gameStates.endGame(player1, player2);
        }

    } else if (gameStates.currentState == "paused") {

        // console.log("The game is paused");
        // gamepads != undefined &&
        if (gamepads.length == 2) {
            // console.log("You can unpause now");

            if (pauseButtonClicked(gamepads)) {
                // console.log("Unpausing now");
                gameStates.changeStateToOnGoing();


            } else {
                // console.log("You didn't click pause"); 
            }

        } else {
            // console.log("Gamepads are disconnected!");
        }
    } else if (gameStates.currentState == "gameOver") {
...
playerControls = function (
        ...
        if (playerA.chosenSpecialAttack == 0) {
            ...
            switch (playerA.chosenSpecialAttack) {
                case 1:
                    ...
                    playerA.changeSpecialIcon(speciaIconImages.shortRangeImg);
                    break;

                case 2:
                    specialAttackName = "Long Slash";
                    playerA.changeSpecialIcon(speciaIconImages.longRangeImg);
                    break;

                case 3:
                    specialAttackName = "Wide Slash";
                    playerA.changeSpecialIcon(speciaIconImages.wideRangeImg);
                    break;
            }
    ...
    if (BBtn && isSpecialFull) {
        ...
        playerA.changeSpecialIcon(speciaIconImages.unknownImg);
    ...
    // console.log("Animation timer: "+playerA.imgTimer);

    if (!playerA.isIdle()) {
        if (playerA.imgTimer < delayDurations.imgDelay) {
            playerA.increaseImgTimer(timeDifferential);

        } else {
            playerA.resetImgTimer();
            playerA.changeFrame(blobImages.idleImg);
        }
    }
};

pauseButtonClicked = function (gamepads) {
    return (gamepads[0].buttons[9].pressed || gamepads[1].buttons[9].pressed)
}
